...
class DotfilesSync
  ...
  def install_ai_skills
    ...
    combined = skill_files.map { |f| File.read(f) }.join("\n")
    header = "# AI Skills\n# Auto-generated by dotfiles-sync — edit files in configs/ai/skills/ instead\n\n"
    content = header + combined

    puts "  📄 Found #{skill_files.length} skill(s): #{skill_files.map { |f| File.basename(f, '.md') }.join(', ')}"
    ...
    unless @dry_run
      ...
      File.write(claude_target, content)
    end
    puts "  ✅ Claude Code: #{claude_target}"

    # Install for Cursor (~/.cursor/rules/skills.mdc)
    cursor_rules_dir = "#{ENV['HOME']}/.cursor/rules"
    cursor_target = "#{cursor_rules_dir}/skills.mdc"
    cursor_frontmatter = "---\ndescription: AI skills from dotfiles\nglobs: \nalwaysApply: true\n---\n\n"
    unless @dry_run
      FileUtils.mkdir_p(cursor_rules_dir)
      File.write(cursor_target, cursor_frontmatter + combined)
    end
    puts "  ✅ Cursor: #{cursor_target}"
  end
